...
def arrange_training(
    ...
    model = hf.AutoModelForCausalLM.from_pretrained(args.brand)
...
def main() -> None:
    ...
    for prompt in tqdm(prompts, desc='Prompts', disable=len(prompts) == 1):
        def prompt_and_tokenize(example: dict) -> dict:
            ...
            model_input = tokenizer(filled_template, truncation=True, padding="max_length", max_length=2048, add_special_tokens=not args.no_input_eos)
            ...
            model_input['labels'] = [
                    tid if tid != tokenizer.pad_token_id else -100
                    for tid in model_input['input_ids']
            ]
            #model_input['labels'] += [-100] * (2048 - len(model_input['labels']))
